...
class RelatorioGenerator:
    ...
    def criar_grafico_barras_tempo(self, dados_infantil, dados_fundamental):
        ...
        if dados_infantil and dados_infantil['tempos']:
            segmentos.append('Educação Infantil')
            tempos_minutos.append(np.mean(dados_infantil['tempos']))
            erros.append(np.std(dados_infantil['tempos']) if len(dados_infantil['tempos']) > 1 else 0)
        
        if dados_fundamental and dados_fundamental['tempos']:
            segmentos.append('Ensino Fundamental')
            tempos_minutos.append(np.mean(dados_fundamental['tempos']))
            erros.append(np.std(dados_fundamental['tempos']) if len(dados_fundamental['tempos']) > 1 else 0)
        ...
        cores = ['#FF6B6B', '#4ECDC4']
        barras = ax.bar(segmentos, tempos_minutos, yerr=erros, capsize=8, 
                        color=cores[:len(segmentos)], alpha=0.8, edgecolor='black', linewidth=1.5)
        
        for bar, tempo in zip(barras, tempos_minutos):
            if tempo > 0:
                ax.text(bar.get_x() + bar.get_width()/2., bar.get_height() + 0.05,
                       f'{tempo:.1f}m', ha='center', va='bottom', fontsize=11, fontweight='bold')
        
        ax.set_ylabel('Tempo Médio (minutos)', fontsize=11, fontweight='bold')
        ax.set_title('Tempo Médio de Avaliação por Aluno', fontsize=13, fontweight='bold', pad=15)
        ax.set_ylim(0, max(tempos_minutos) * 1.3 if max(tempos_minutos) > 0 else 1)
        ax.grid(axis='y', alpha=0.3, linestyle='--')
        ax.set_facecolor('#f8f9fa')
